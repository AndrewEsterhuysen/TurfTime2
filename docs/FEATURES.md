...
| `FieldViewPlacement` | Interactive Field View: 4×4 pitch cells; drag or tap-to-place any token (Setup: single-tap arm/move, double-tap rename; Absent = move-only, else swap); new players start on Bench; roster `fieldCell` sync (schema v3); pair outline colours | Free | Reuses `SetPlayerPosition` / `PlaceOrSwapOnFieldCell` / `SwapPlayerRoles`; never gate |
