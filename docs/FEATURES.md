...
| `FieldViewPlacement` | Interactive Field View: unpositioned stack, 4×4 pitch cells, drag to Field/Bench/Goalie; roster `fieldCell` sync (schema v3); pair outline colours | Free | Reuses `SetPlayerPosition` / `PlaceOrSwapOnFieldCell`; branch `field-view-player-positioning`; never gate |
